'use strict';

var J = w,
  q = w;
(function (z, B) {
  var f = {
      z: 0x1b1,
      B: 0x1c4,
      O: 0x1b7,
      I: 0x18d,
      r: 0x194,
      d: 0x184,
      t: 0x17f,
      c: 0x1c1,
      m: 0x189,
      U: 0x1b0
    },
    X = w,
    R = w,
    O = z();
  while (!![]) {
    try {
      var I = parseInt(X(f.z)) / 0x1 + parseInt(X(f.B)) / 0x2 * (-parseInt(R(f.O)) / 0x3) + parseInt(X(f.I)) / 0x4 * (-parseInt(R(0x1ab)) / 0x5) + -parseInt(X(f.r)) / 0x6 * (-parseInt(X(f.d)) / 0x7) + -parseInt(R(f.t)) / 0x8 * (-parseInt(R(f.c)) / 0x9) + -parseInt(X(f.m)) / 0xa + -parseInt(R(0x1b6)) / 0xb * (parseInt(X(f.U)) / 0xc);
      if (I === B) break;else O['push'](O['shift']());
    } catch (r) {
      O['push'](O['shift']());
    }
  }
})(D, 0x73c3a);
Object[J(0x191) + q(0x192)](exports, '__esModu' + 'le', {
  'value': !0x0
}), exports[q(0x18e)] = void 0x0;
function w(z, B) {
  var O = D();
  return w = function w(I, r) {
    I = I - 0x178;
    var d = O[I];
    return d;
  }, w(z, B);
}
function D() {
  var De = ['isEmotic', '_type', '_channel', '147792rlptXv', '796445uxBynR', 'text', 'shout', 'SxiIf', 'bIdcB', '715NSVozb', '18AjJPPj', 'ticon', 'dMember', 'isMap', 'getUnrea', '_user', 'pleUsers', 'type', 'GbQCt', 'originTy', '207bElpQG', 'getMulti', 'Fcfzt', '436hedsoE', '_isHidde', 'KilLe', 'pdOWx', 'pQsEK', 'chat_id', 'hoto', 'hat', '3992vHuefG', 'indexOf', '../../ut', 'UcRKA', 'sendTime', '2026906eIGiCt', 'toJSON', 'map', 'prototyp', 'getReadM', '3242320LbdodS', 'ByIDs', 'DBUtil', 'raw', '5176irvdxZ', 'Chat', 'mbers', 'user_id', 'definePr', 'operty', 'isDelete', '18uDJIgD', 'getNextC', 'BlHJw', 'til', 'mSUjg', 'ASRui', '_mention', 'isMultiP', 'hasShout', 'isFile', 'attachme', 'uYtRh', 'mentions', 'ers', 'isAudio', 'isProfil', 'readMemb', 'HHxzU', 'isPhoto', 'ember', 'isReply', 'uDJJN', 'getPrevC', '295UjXtNf', '_raw'];
  D = function D() {
    return De;
  };
  return D();
}
var db_util_1 = require(q(0x181) + 'ils/db_u' + J(0x197)),
  Chat = function () {
    var Dn = {
        z: 0x1c0,
        B: 0x18f,
        O: 0x1a4,
        I: 0x18c,
        r: 0x183,
        d: 0x187,
        t: 0x199,
        c: 0x191,
        m: 0x192,
        U: 0x191,
        De: 0x1b4,
        DA: 0x191,
        DQ: 0x187,
        DS: 0x1a9,
        Da: 0x191,
        Di: 0x185,
        Ds: 0x192,
        DN: 0x17a,
        DV: 0x191,
        Dp: 0x19e,
        DW: 0x1ad,
        Dl: 0x193,
        DY: 0x19b,
        Dk: 0x17d,
        DL: 0x1b8,
        Dv: 0x187,
        Dg: 0x1a6,
        Dx: 0x1a8,
        Df: 0x187,
        DM: 0x1ba,
        Dy: 0x17e,
        w0: 0x187,
        w1: 0x1a0
      },
      DP = {
        z: 0x19a,
        B: 0x1ac,
        O: 0x19e,
        I: 0x182,
        r: 0x18b,
        d: 0x1c2,
        t: 0x1bd
      },
      DE = {
        z: 0x190
      },
      DC = {
        z: 0x196,
        B: 0x1b5,
        O: 0x17e,
        I: 0x18b,
        r: 0x195,
        d: 0x17e
      },
      Dh = {
        z: 0x19f,
        B: 0x19f,
        O: 0x198,
        I: 0x195,
        r: 0x18b
      },
      DU = {
        z: 0x1ac
      },
      Dm = {
        z: 0x179,
        B: 0x1ac
      },
      Dc = {
        z: 0x1bc,
        B: 0x1af
      },
      Dt = {
        z: 0x19f,
        B: 0x1ac,
        O: 0x196,
        I: 0x1b3
      },
      Dd = {
        z: 0x1ac
      },
      Dr = {
        z: 0x1ac
      },
      DO = {
        z: 0x18b,
        B: 0x1a7,
        O: 0x18c,
        I: 0x18c
      },
      DB = {
        z: 0x1b9,
        B: 0x17c,
        O: 0x18c
      },
      Dz = {
        z: 0x1bc
      },
      Dw = {
        z: 0x1bc
      },
      DD = {
        z: 0x1af
      },
      D9 = {
        z: 0x1af
      },
      D8 = {
        z: 0x1ae
      },
      D7 = {
        z: 0x1ae,
        B: 0x1be,
        O: 0x178
      },
      K = J,
      H = q,
      z = {
        'EnKqF': function EnKqF(O, I) {
          return O >= I;
        },
        'GbQCt': function GbQCt(O, I) {
          return O & I;
        },
        'UcRKA': function UcRKA(O, I) {
          return O !== I;
        },
        'uYtRh': function uYtRh(O, I) {
          return O === I;
        },
        'BlHJw': function BlHJw(O, I) {
          return O === I;
        },
        'KilLe': function KilLe(O, I) {
          return O(I);
        },
        'mSUjg': function mSUjg(O, I) {
          return O < I;
        },
        'bIdcB': function bIdcB(O, I) {
          return O < I;
        },
        'Fcfzt': function Fcfzt(O, I) {
          return O - I;
        },
        'ASRui': K(Dn.z) + 'pe',
        'bGIvy': 'channel',
        'SxiIf': 'user',
        'pQsEK': 'unreadMe' + H(Dn.B),
        'BfsPX': H(Dn.O) + K(0x1a1),
        'uDJJN': H(0x1b2),
        'HHxzU': K(Dn.I),
        'pdOWx': K(Dn.r)
      };
    function B(O) {
      var Z = H,
        u = H;
      this['_isHidde' + 'n'] = !0x1, this[Z(D7.z)] = -0x1, this['_raw'] = O;
      var I = O[Z(D7.B)];
      this[u(D7.O) + 'n'] = z['EnKqF'](this['_type'], 0x4000), this[Z(D7.z)] = z[Z(0x1bf)](0xffffbfff, I);
    }
    return Object['definePr' + 'operty'](B[K(Dn.d) + 'e'], z[H(Dn.t)], {
      'get': function get() {
        var G = H;
        return this[G(D8.z)];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[H(Dn.c) + K(Dn.m)](B[K(Dn.d) + 'e'], z['bGIvy'], {
      'get': function get() {
        var j = H;
        return this[j(D9.z)];
      },
      'set': function set(O) {
        var o = H;
        this[o(DD.z)] = O;
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[H(Dn.U) + H(0x192)](B[H(Dn.d) + 'e'], z[H(Dn.De)], {
      'get': function get() {
        var F = K;
        return this[F(Dw.z)];
      },
      'set': function set(O) {
        var b = H;
        z['UcRKA'](null, O) && (this[b(Dz.z)] = O);
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[K(Dn.DA) + 'operty'](B[K(Dn.d) + 'e'], z[K(0x17b)], {
      'get': function get() {
        var h = K,
          C = H;
        return db_util_1[h(0x18b)][C(0x1bb) + h(DB.z)](this[C(0x18c)][h(DB.B)], this[C(DB.O)]['id']);
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object['definePr' + 'operty'](B[H(Dn.d) + 'e'], z['BfsPX'], {
      'get': function get() {
        var E = H,
          T = K;
        return db_util_1[E(DO.z)][T(0x188) + E(DO.B)](this[E(DO.O)]['chat_id'], this[T(DO.I)]['id']);
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object['definePr' + 'operty'](B[H(Dn.DQ) + 'e'], z[K(Dn.DS)], {
      'get': function get() {
        return this['_raw']['message'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[H(0x191) + 'operty'](B[K(0x187) + 'e'], 'id', {
      'get': function get() {
        var P = H;
        return this[P(Dr.z)]['id'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[K(Dn.Da) + K(0x192)](B[H(0x187) + 'e'], z[K(0x1a5)], {
      'get': function get() {
        var n = H;
        return this[n(Dd.z)];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), B[H(0x187) + 'e'][K(0x19c)] = function () {
      var e = H,
        A = K,
        O;
      return z[e(Dt.z)](!0x0, z[A(0x19f)](null, O = this[A(Dt.B)]['attachme' + 'nt']) || z[A(Dt.O)](void 0x0, O) ? void 0x0 : O[A(Dt.I)]);
    }, B[H(Dn.d) + 'e'][K(Dn.Di)] = function () {
      var Q = K,
        S = K;
      return {
        'chat': this['_raw'],
        'user': this[Q(Dc.z)],
        'channel': this[Q(Dc.B)]
      };
    }, Object[K(Dn.c) + H(Dn.Ds)](B['prototyp' + 'e'], z[H(Dn.DN)], {
      'get': function get() {
        var a = H,
          i = K;
        return new Date(0x3e8 * z[a(Dm.z)](Number, this[a(Dm.B)]['created_' + 'at']));
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), Object[H(Dn.DV) + K(0x192)](B[H(Dn.DQ) + 'e'], K(Dn.Dp) + 'nt', {
      'get': function get() {
        var s = K,
          N = H;
        return this[s(DU.z)][s(0x19e) + 'nt'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), B[H(0x187) + 'e'][K(Dn.DW) + 'on'] = function () {
      return !0x1;
    }, B[K(Dn.DQ) + 'e'][K(Dn.Dl) + 'd'] = function () {
      var V = K;
      return this[V(0x178) + 'n'];
    }, B[H(Dn.d) + 'e']['isFeed'] = function () {
      return !0x1;
    }, B['prototyp' + 'e'][K(Dn.DY) + H(Dn.Dk)] = function () {
      return !0x1;
    }, B[H(Dn.DQ) + 'e']['isNormal'] = function () {
      return !0x1;
    }, B[H(0x187) + 'e']['isOldEmo' + H(Dn.DL)] = function () {
      return !0x1;
    }, B[K(Dn.Dv) + 'e'][H(Dn.Dg)] = function () {
      return !0x1;
    }, B[H(Dn.Dv) + 'e'][K(Dn.Dx)] = function () {
      return !0x1;
    }, B['prototyp' + 'e'][K(0x19d)] = function () {
      return !0x1;
    }, B[H(Dn.DQ) + 'e']['isVideo'] = function () {
      return !0x1;
    }, B[H(Dn.Df) + 'e'][H(0x1a2)] = function () {
      return !0x1;
    }, B['prototyp' + 'e'][K(Dn.DM)] = function () {
      return !0x1;
    }, B['prototyp' + 'e'][K(0x1a3) + 'e'] = function () {
      return !0x1;
    }, B[H(Dn.DQ) + 'e']['getPrevC' + H(Dn.Dy)] = function (O) {
      var p = K,
        W = H;
      return z[p(Dh.z)](void 0x0, O) && (O = 0x1), z[p(Dh.B)](0x0, O) ? this : z[p(Dh.O)](O, 0x0) ? this[W(Dh.I) + W(0x17e)](-O) : db_util_1[p(Dh.r)][W(0x1aa) + W(0x17e)](this, O);
    }, B['prototyp' + 'e'][K(0x195) + H(Dn.Dy)] = function (O) {
      var l = K,
        Y = K;
      return void 0x0 === O && (O = 0x1), z[l(DC.z)](0x0, O) ? this : z[Y(DC.B)](O, 0x0) ? this[l(0x1aa) + Y(DC.O)](-O) : db_util_1[l(DC.I)][Y(DC.r) + Y(DC.d)](this, O);
    }, Object[K(0x191) + 'operty'](B[H(Dn.w0) + 'e'], K(Dn.w1), {
      'get': function get() {
        var DT = {
            z: 0x1c3,
            B: 0x180
          },
          k = K,
          L = K;
        if (!this[k(DP.z) + 's']) {
          var O = this[L(DP.B)][k(DP.O) + 'nt'];
          if (null !== O && z[k(DP.I)](void 0x0, O['mentions'])) {
            var I = O['mentions'][k(0x186)](function (m) {
                var v = k;
                return m[v(DE.z)];
              }),
              d = Array['from'](new Set(I)),
              c = db_util_1[L(DP.r)][L(DP.d) + L(DP.t) + k(0x18a)](d, !0x1);
            c['sort'](function (m, U) {
              var g = k,
                x = L;
              return z[g(DT.z)](I[x(DT.B)](m['id']), I['indexOf'](U['id']));
            }), this['_mention' + 's'] = c;
          } else this[L(DP.z) + 's'] = [];
        }
        return this['_mention' + 's'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), B;
  }();
exports[J(0x18e)] = Chat;