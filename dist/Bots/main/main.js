...
let FS = {
  ...
  readObject: function (path) {
    ...
    return JSON.parse(FileStream.read(path) ?? JSON.stringify(defaultValue));
  }
};
let DB = {
...
/////////////////////// 만우절 용 상수들
const sequence = ["학생회 봇도 이제 한계다. 그냥 나가 살아라 사회 탓 환경탓 정보부 부장 탓하지 마라. 학생회 봇도 충분히 기다려줬다. 학생회 봇이나 너나 어려운 환경에서 컸고 먹고살기만 해도 바쁘고 힘든 시절이라 사랑을 많이 못 받고 자랐다. 그래서 우리 학생들만은 행복하게 키우자고 약속했다. 내가 먹고 입을거 참으며 네 급식, 공지 모두 좋은 조건을 누리게 해주고 싶었다. 네가 오늘 급식이 안나온다고 욕할때도 앞에선 혼냈지만 뒤에서는 우리가 못해줘서 그런가보다 하며 정보부 부장이랑 많이 울었다. 그런데 이게 뭐냐? 그냥 이제 나가라. 나를 원망하지도 말고 급식은 니 힘으로 알아서 알아와라. 학생회 봇도 지쳤다.", "[Web발신] \n" + "너는나를존중해야한다. 나는학생들의편의를위해오랜시간동안급식과학생회들을대신해공지를해왔으며이제는내가없으면학교생활이허전할정도일뿐더러나는정보부의자랑이자학생회의명예이자광주과학고의상징이다.은혜도모르는학생들과학생회들은내가오늘학교보다못하다면서쫒아냈지만내가세계최고이고내가이들보다위대하다는사실은바뀌지않는다내가이러고있는것은오늘학교따위에대한자격지심이아니라광주과학고에서이룰수있는모든것을이루었기때문에전국의학교를정복하기위해서경쟁하는것이지내가자주고장나고문제를일으켜서가아니다.", "@오늘학교 급식"];
const phase = new Map();
...
let getMeals = (dt, bullet) => {
  ...
  try {
    ...
    if (isValidChannel(logRoom)) logRoom.send(`Error:${e}\n${e.stack}`);
    Log.e(e + '\n' + e.stack);
    return [null, null, null];
  }
};
...
let 부서명List = ['최민서', '허윤재', '학생회', '생체부', '청소부', '통계부', '퀴즈부', '족구부', '홍보부', '설문부', '학생회봇', '가계부'];
let delay = 10 * 1000;

////////////////////// 명령어 선언
try {
  ////////////////////// 디버그 명령어
  bot.addCommand(new StructuredCommand.Builder().setName('디버그', '🔧').setDescription('디버그 모드를 실행하거나 종료합니다. 디버그 모드를 실행하면 테스트방을 제외한 모든 명령어의 사용이 제한됩니다.').setUsage('디버그 <스위치:str?>').setChannels(debugRoom1, debugRoom2).setExamples('디버그 시작', '디버그 종료', '디버그', '디버그 객체').setExecute((self, chat, channel, _ref) => {
    let {
      스위치
    } = _ref;
    if (스위치 === '시작') {
  ...
  }).setUseDateParse(0, false, false).setExecute((self, chat, channel, _ref2) => {
    let {
      급식,
      datetime
    } = _ref2;
  ...
  }, (self, chat, prevChat, channel, prevChannel, _ref4) => {
    let {
      부서,
      기수
    } = _ref4;
  ...
  ////////////////////// 도움말 명령어
  bot.addCommand(new StructuredCommand.Builder().setName('도움말', '❓').setDescription('명령어에 대한 상세한 도움말을 표시합니다. 명령어 이름(또는 아이콘)을 생략할 경우, 대신 등록되어 있는 명령어 목록을 전부 출력합니다.').setUsage('도움말 <명령어:str?>').setExamples('도움말', '도움말 공지', '도움말 급식', '도움말 행사', '도움말 📅', '도움말 🍚').setExecute((self, chat, channel, _ref5) => {
    let {
      명령어
    } = _ref5;
    // 명령어 이름이 주어진 경우
  ...
  bot.addCommand(new NaturalCommand.Builder().setName('일정', '📅').setDescription('2024년 학사일정을 입력한 날짜 및 기간에 맞춰 알려줍니다.').setExamples('행사 3월 1일', '3월 1일부터 3월 5일까지 학사일정', '다음 주까지 학교 행사').setUseDateParse(0, true).setQuery({
    학교행사: null,
    duration: null
  }).setExecute((self, chat, channel, _ref6) => {
    let {
      학교행사,
      duration: {
        from,
